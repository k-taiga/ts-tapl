  ...
  switch (ty2.tag) {
    ...
    case "Object": {
      ...
      for (const prop2 of ty2.props) {
        ...
        const prop1 = ty1.props.find(function (prop1: { name: string; }) {
  ...
  switch (t.tag) {
    ...
    case "objectNew": {
      const props = t.props.map(
        ...
        function ({ name, term }) {
          return { name, type: typecheck(term, tyEnv) };
        }
      );
    ...
    case "objectGet": {
      ...
      const prop = objectTy.props.find(function (prop: { name: string; }) {
...
function subtype(ty1: Type, ty2: Type): boolean {
  switch (ty2.tag) {
    ...
    case "Func": {
      if (ty1.tag !== "Func") return false;
      for (let i = 0; i < ty1.params.length; i++) {
        // ty2の仮引数がty1の仮引数の部分型かどうか、引数の順番毎に確認
...
console.log(
  typecheck(
    parse(`
    type F = function(): { foo: number; bar: boolean};
    const f = function(x: F) { return x().bar; };
    const g = function() { return {foo: 1}; };
    f(g);`),
    {}
  )
);
